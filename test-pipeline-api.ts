/**
 * API Test Script - Tests the refactored pipeline
 * Runs queries like a normal user would and monitors results
 */
...
async function testQuery(query: string, index: number) {
  ...
  console.log(`\n${'='.repeat(60)}`);
  console.log(`📝 Test ${index + 1}/${TEST_QUERIES.length}: "${query}"`);
  ...
  try {
    ...
    if (result.productCards && result.productCards.length > 0) {
      console.log(`   🎯 First product: ${result.productCards[0].title?.substring(0, 60)}...`);
      console.log(`   💰 First product price: $${((result.productCards[0].priceCents || 0) / 100).toFixed(2)}`);
    }
    
    if (result.replyText) {
      console.log(`   💬 Reply preview: ${result.replyText.substring(0, 100)}...`);
    }
...
async function runTests() {
  console.log('🚀 Starting API Pipeline Tests...\n');
  ...
  console.log('\n📋 Detailed Results:');
  results.forEach((r, i) => {
    console.log(`  ${i + 1}. "${r.query.substring(0, 50)}..." - ${r.success ? '✅' : '❌'} - ${r.productCount} products - ${(r.duration / 1000).toFixed(2)}s`);
  });
  
  console.log('\n💡 Next steps:');
  console.log('   1. Review logs in app.log for detailed execution traces');
  console.log('   2. Check for pipeline steps: gender extraction, category filtering, refinement, retrieval');
  console.log('   3. Verify gender filtering is working correctly');
  console.log('   4. Monitor constraint relaxation behavior');
